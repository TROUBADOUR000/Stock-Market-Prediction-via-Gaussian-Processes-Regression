...
class Wrapper:
    ...
    def __init__(self, company_name: str):
        ...
        # kernel = 100 * Matern(length_scale=0.2, nu=2.5) # figure2
        kernel = 80 * RBF(length_scale=1)   # figure1
    ...
    def get_eval_model(self, start_year: int, end_year: int, pred_year: int, pred_quarters: list = None):
        ...
        if pred_quarters is not None:
            pred_days = [day for day in range(63 * (pred_quarters[0]-1), 63 * pred_quarters[int(len(pred_quarters) != 1)])]
        else:
